...
param(
    ...
    [Parameter(Mandatory = $true)][string]$WboxLinux,
    [Parameter(Mandatory = $true)][string]$UbuntuImage
)
...
$arguments = @('run', (Join-Path $PSScriptRoot 'rhai\test-windows-ubuntu.rhai'),
    '--cwd', $repoRoot, '--project-root', $repoRoot, '--timeout-ms', '3600000', '--',
    $Wbox, $WboxLinux, $UbuntuImage)
& $runtime @arguments
exit $LASTEXITCODE
