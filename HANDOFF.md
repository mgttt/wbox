...
F9.1–F9.39 全部落地并有持续门禁。近期这一串是本轮做的：
...
| F9.39 多阶段构建 | MS.1–MS.5 | 实现是「每个 `FROM` 切换产物目录」而非重构成多次构建，指令执行那段一个字没动；多阶段禁用前缀缓存（跨阶段复用是**错的**缓存）；判据两条：产物拿到了 **且** 中间阶段没进最终镜像 |
...
### 「一个是另一个的特例」的两份函数，必然漏改一个

补 `--entrypoint` 时我留了 `merged_command`（旧）与 `merged_command_with`（新）
两个函数，前者是后者传 `None` 的薄包装。看着无害，结果**当场就漏改了一个调用点**
——`create` 那条路改了、`run` 那条没改，单测全绿，实测才发现 `--entrypoint` 不生效。
...
AF.3 当场变红——这正是它该做的：提醒我「文档里那句『不支持』现在是谎话了」。
门禁在这里的作用不是断言「这样是对的」，而是**锁住已写进文档的行为**，
让实现和文档不能各走各的。（PZ.6 那条 `exec` 进暂停容器的差异同理。）

红了之后要做的是**更新断言去覆盖新的真实约束**（AF.3 改成盯「`--from` 的源必须
是绝对路径」），而不是删掉了事。
...
**Q3 的 F9 序列已全部做完**（F9.1–F9.39），多阶段构建也已补上。剩下的都在天花板之外或属另一象限：
...
§2.4 的四象限表已逐行核对过实况；**§2.4.1「每格是怎么跑起来的」**把四象限
拆成「两条隔离链路 × 两种程序格式」，读它就能推出一个新能力该落在哪几格
（依赖 overlay 的只在 Q3；依赖 `/proc`/进程树的 Q3+Q4 都成立）。

**§2.4.2「与四个参照物的架构对照」**逐格写清对标物的介入点（Sandboxie 的内核
minifilter、WSL2 的 Hyper-V VM、Podman 的 namespace、Wine 的 ABI 翻译）、wbox 的
