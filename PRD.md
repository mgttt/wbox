...
| Q1.3 | 临时目录私有化（`TEMP`/`TMP` 指向容器私有目录）| **可做**（待 Windows 侧确认）| 纯环境变量，不需驱动。能覆盖相当一部分「程序把临时文件乱写到用户目录」的场景。判据：容器内进程写临时文件落在私有目录，退出后随容器一并清理 |
...
**Q1.3–Q1.5 标着「待 Windows 侧确认」不是客套**：本文由 Linux 侧 agent 写，
...
├── W6 Q1 临时目录私有化（TEMP/TMP）          [Windows agent] 待验证可行性后实现
...
**W6 临时目录私有化**。把容器内的 `TEMP`/`TMP` 指向容器私有目录。
- 为什么值得做：Q1 没有写重定向（§2.4.2），但「程序把临时文件乱写到用户目录」
  是那一格最常见的困扰之一，而这条纯靠环境变量就能挡掉相当一部分。
- 判据：容器内进程创建临时文件，落在私有目录内；容器退出后随状态目录一并清理；
  不设该变量时行为不变。
- 注意：这只覆盖**遵守 `TEMP`/`TMP` 约定**的程序。硬编码路径的程序挡不住——
  文档里要写明这条边界，别让它被读成「临时文件一定被隔离」。
