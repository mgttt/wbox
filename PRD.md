...
**F9.2 端口映射**（取证部分完成，**尚未排期实现**）。

**本机取证的边界要先说清**：开发机上没有 `ip`、`slirp4netns`、`pasta`，
因此 veth 与 slirp 路线**我没有实机验证过**，下面第 1、2 条是依据机制的推断，
不是实测结论。第 3 条不同——它依赖的 `setns` 能力已由 F8.4 `exec` 实测走通。

1. **veth/网桥：rootless 下不可行**。建 veth 对需要宿主 netns 里的
   `CAP_NET_ADMIN`，而 wbox 的前提就是不要 root。
2. **slirp4netns / pasta：可行但引入外部依赖**。这是 Podman rootless 的标准
   答案，代价是用户得先装。与 §2.2「Portable：免安装」直接冲突——**可以作为
   可选加速路径，但不能当作唯一实现**，否则 wbox 在裸机上就没有端口映射。
3. **wbox 自己做用户态转发：无外部依赖，推荐路线**。supervisor 留在宿主
   netns，同时**有能力 `setns` 进容器 netns**——这一点不是推测，F8.4 的
   `exec` 已经实测走通（P.20 断言 exec 与容器同 netns）。于是可以：宿主侧监听
   `-p` 指定的端口，收到连接后由一个 `setns` 进容器 netns 的助手连到容器内
   端口，两端对拷。

**第 3 条要解决的真问题**（排期前必须有答案，别先写 CLI 旗标）：

- 跨进程传递 fd 需要 `SCM_RIGHTS`，还是每条连接现 fork 一个转发助手更简单？
  前者省进程，后者省复杂度——先量一下连接频次再定。
- **只覆盖 TCP**。UDP 与 ICMP 用这套做不了，文档必须写明，不能让用户以为
  `-p` 等价于 Docker 的 `-p`。
- 与 `--allow-network`（不建 netns，直接共享宿主网络）的语义冲突：同时给出时
  应当报错而不是二选一静默。

**结论**：路线 3 可行且无依赖，但工作量不小且能力有边界（仅 TCP）。
在它落地前，`-p` 继续**明确报错**并指向本节，不做半吊子实现。
