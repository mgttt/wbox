...
| Windows × Linux 镜像 | WSL2 / Docker Desktop | 已有 `build` 子集；无卷挂载/端口映射，**性能不可比** |
| Linux × Linux 镜像 | Podman / Docker | 已有 `-v`/`-p`（仅 TCP）/`build`+分层缓存/`--restart`/`--user`（数字 id）；无 compose、push、自定义网络 |
