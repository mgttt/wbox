...
| Linux × Linux 镜像 | Podman / Docker | 已有 `-v`/`-p`（仅 TCP）/`build`+分层缓存/`--restart`/`--user`（数字 id）/`--cap-add`/`--cap-drop`/`--seccomp-deny`（拒绝名单）/healthcheck/`--network container:`/overlay 可写层/`push`（原样回推，保留分层）/`diff`/`commit`/`pause`/`save`·`load`/`export`·`import`/`cp`/`stats`/`restart`/`rename`·`prune`/`compose` 子集/IPC·UTS 隔离与共享；无自定义 bridge 网络与内建 DNS、镜像分层存储 |
