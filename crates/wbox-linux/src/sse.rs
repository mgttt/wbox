...
use crate::machine::{ExecResult, Machine};

/// 取 128 位操作数：xmm 寄存器或 16 字节内存。
fn read_xmm_rm(m: &mut Machine, d: &Dec, rm: Rm) -> ExecResult<[u8; 16]> {
    match m.fixup(d, rm) {
...
fn write_xmm_rm(m: &mut Machine, d: &Dec, rm: Rm, v: [u8; 16]) -> ExecResult<()> {
    match m.fixup(d, rm) {
...
fn read_narrow(m: &mut Machine, d: &Dec, rm: Rm, n: usize) -> ExecResult<[u8; 16]> {
    let mut out = [0u8; 16];
    match m.fixup(d, rm) {
...
fn write_narrow(m: &mut Machine, d: &Dec, rm: Rm, v: &[u8], n: usize) -> ExecResult<()> {
    match m.fixup(d, rm) {
        Rm::Reg(r) => {
...
pub fn exec(m: &mut Machine, d: &mut Dec, op: u8) -> ExecResult<()> {
    match op {
