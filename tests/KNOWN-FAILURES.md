...
| 环境 | 当前基线 | 失败项 |
|---|---|---|
| 真 Windows（本机，native） | 19 个用例：**19 PASS / 0 FAIL / 0 SKIP** | 无 |
| wine（当前机器基线） | `t_net_sockopt @wine` | 新增 `t_eventfd`、`t_signal_timer`、`t_timerfd` 待复核 |

真机断言级统计为 **634 条：pass=625 fail=0 skip=9**；skip 均为 symlink
