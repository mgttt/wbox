...
| ~~G~~ | ~~mmap 精度~~ —— **已修，整组清空**（映射记账 + MAP_SHARED 在 munmap/msync/mremap/MAP_FIXED 覆盖时回写，失败报 EIO 且保持映射） | — |
...
| 用例 | 标注 | 原因 |
|---|---|---|
| `t_path` | `@windows` | volume/file identity 已让 hardlink 的 `ino/nlink` 断言通过；剩余 4 项是 `unlink(directory)` 映射 `EACCES`，以及 Win32 不接受文件名中的双引号（三条断言）。Linux 宿主上通过。 |
