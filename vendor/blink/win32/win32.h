...
// Flush/sync MAP_SHARED file-backed pages to a snapshot parent.
...
enum {
  ...
  W32FD_SIGNALFD, // signalfd pending set wrapped in a CRT fd (w32fd.c table)
...
int WboxSockDup(int fd);                    // -2 = not a socket
int WboxSockDup2(int fd, int fd2);          // -2 = not a socket
