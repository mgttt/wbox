...
| 7 | MAP_SHARED 文件写回 | ✅ 文件映射写回按文件 ID/64 位偏移同步，注册表按需增长；匿名共享映射用未链接临时文件承载并走同一 Fshare 路径。msync/munmap/MAP_FIXED 写回错误返回 guest 且保留原映射，并在 child 条目移除前按范围更新父窗口。覆盖 160 个并存普通/匿名映射、4 GiB+ 稀疏文件映射与低位别名隔离、fork 后父窗口可见性、子映射搬移、exec wipe、固定覆盖及内部 fd 0；由 `t_mmap` / `t_fork_mem` / `t_exec` 验证 |
