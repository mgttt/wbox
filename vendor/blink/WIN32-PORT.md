...
| 3 | socket 族 | ✅ Winsock2 映射：AF_INET/INET6 STREAM/DGRAM、pathname AF_UNIX stream、匿名 AF_UNIX stream/datagram socketpair、epoll/poll、errno 与 O_NONBLOCK。socketpair 内部 loopback 承载层在 Hostfs 对外保持未命名 AF_UNIX 身份；epoll 对象与 CRT fd alias 分离，fork/dup 后共享 interest table，并由 per-System 表分配 guest fd |
...
| 6 | mremap | ✅ 匿名及文件映射支持原地扩缩、`MREMAP_MAYMOVE` / `MREMAP_FIXED` 搬移、数据与逐页权限保留、新增页加载/清零；文件 backing 独立于原 fd 生命周期，MAP_PRIVATE 脏页隔离和 MAP_SHARED 写回均覆盖；4 GiB+ 文件 offset 保持 64 位 |
| 7 | MAP_SHARED 文件写回 | ✅ 文件映射写回按文件 ID/64 位偏移同步，注册表按需增长；匿名共享映射用未链接临时文件承载并走同一 Fshare 路径。msync/munmap/MAP_FIXED 写回错误返回 guest 且保留原映射，并在 child 条目移除前按范围更新父窗口。覆盖 160 个并存普通/匿名映射、4 GiB+ 稀疏文件映射与低位别名隔离、fork 后父窗口可见性、子映射搬移、exec wipe、固定覆盖及内部 fd 0；64 位 positioned I/O 同时供高位 sendfile offset 使用；由 `t_fd_rw` / `t_mmap` / `t_fork_mem` / `t_exec` 验证 |
...
5. **文件 MAP_SHARED**：写回注册按 VA window 隔离并持有独立 backing fd；
   ...
   Windows 的 `MAP_SHARED|MAP_ANONYMOUS` 由立即 unlink 的临时文件实现，
   因此同样按文件 ID/偏移走 Fshare；旧 Shseg 重复地址表已删除，避免两套
   元数据在扩容或拆分失败后分叉。
   child `msync` 会立即按请求范围更新仍存活的父窗口；`munmap` 和
   `MAP_FIXED` 则在删除 child Fshare 条目前执行同样的范围同步，避免磁盘
   已写回但父映射因同步来源消失而永久陈旧。
